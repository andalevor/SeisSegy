...
static SeisSegyErrCode write_trace_samples_var(SeisOSegy* sgy,
    SeisTrace const* t);
static SeisSegyErrCode write_trace_rev0(SeisOSegy* sgy, SeisTrace const* t);
static SeisSegyErrCode write_trace_rev1(SeisOSegy* sgy, SeisTrace const* t);
static SeisSegyErrCode write_trace_rev2(SeisOSegy* sgy, SeisTrace const* t);
...
void seis_osegy_unref(SeisOSegy* sgy)
{
    if (!--sgy->rc) {
        if (!sgy->com->err.code)
            write_bin_header(sgy); /* update samp_per_tr value */
...
SeisSegyErrCode seis_osegy_open(SeisOSegy* sgy, char const* file_name)
{
    ...
    sgy->com->samp_buf = (char*)malloc(com->samp_per_tr * com->bytes_per_sample);
    if (!sgy->com->samp_buf) {
        com->err.code = SEIS_SEGY_ERR_NO_MEM;
        com->err.message = "can't allocate memory in osegy open";
        goto error;
    }
    switch (sgy->com->bin_hdr.SEGY_rev_major_ver) {
    case 0:
        sgy->write_trace = write_trace_rev0;
        break;
    }
error:
    return com->err.code;
...
void seis_osegy_add_ext_text_header(SeisOSegy* sgy, char* hdr)
{
    ...
    if (!seis_common_segy_get_text_headers_num(com)) {
        char const* hdr;
        switch (com->bin_hdr.max_num_add_tr_headers) {
        case 0:
        default:
...
{
    return sgy->write_trace(sgy, trc);
}

SeisSegyErrCode write_trace_rev0(SeisOSegy* sgy, SeisTrace const* t)
{
    SeisSegyErr const* err = seis_osegy_get_error(sgy);
    TRY(write_trace_header(sgy, seis_trace_get_header_const(t)));
...
SeisSegyErrCode write_text_header(SeisOSegy* sgy)
{
    ...
    if (!hdr_num) {
        ...
        case 0:
        default:
            strncpy(buf, seis_segy_default_text_header_rev0, TEXT_HEADER_SIZE);
            break;
        case 1:
            strncpy(buf, seis_segy_default_text_header_rev1, TEXT_HEADER_SIZE);
            break;
        case 2:
            strncpy(buf, seis_segy_default_text_header_rev2, TEXT_HEADER_SIZE);
            break;
        }
        ascii_to_ebcdic(buf);
        hdr = buf;
...
SeisSegyErrCode write_trailer_stanzas(SeisOSegy* sgy)
{
    ...
    for (size_t i = 1; i < num; ++i) {
        ...
        if (written != TEXT_HEADER_SIZE) {
            com->err.code = SEIS_SEGY_ERR_FILE_WRITE;
            com->err.message = "i/o error on trailer stanza writing";
        }
    }
    return com->err.code;
}
    ...
    SeisTraceHeader const* hdr)
{
    ...
    add_trc_hdr_num = add_trc_hdr_num ? add_trc_hdr_num : com->bin_hdr.max_num_add_tr_headers;
    ...
    if (written != TRACE_HEADER_SIZE) {
        com->err.code = SEIS_SEGY_ERR_FILE_WRITE;
        com->err.message = "i/o error on additional trace header writing";
        goto error;
    }
...
SeisSegyErrCode write_trace_samples_fix(SeisOSegy* sgy, SeisTrace const* t)
{
    ...
    size_t written = fwrite(com->samp_buf, 1,
        com->bytes_per_sample * com->samp_per_tr, com->file);
...
SeisSegyErrCode write_trace_samples_var(SeisOSegy* sgy, SeisTrace const* t)
{
    ...
    if (*samp_num != com->samp_per_tr) {
        ...
        if (!com->samp_buf) {
            com->err.code = SEIS_SEGY_ERR_NO_MEM;
            com->err.message = "can't get memory to write variable trace";
            goto error;
        }
...
{
    uint32_t tmp = val;
    val = (tmp & 0xff) << 24 | (tmp & 0xff000000) >> 24 | (tmp & 0xff00) << 8 | (tmp & 0xff0000) >> 8;
    memcpy(*buf, &val, sizeof(int32_t));
    *buf += sizeof(int32_t);
}
...
{
    uint64_t tmp = val;
    val = (tmp & 0xff) << 56 | (tmp & 0xff00000000000000) >> 56 | (tmp & 0xff00) << 40 | (tmp & 0xff000000000000) >> 40 | (tmp & 0xff0000) << 24 | (tmp & 0xff0000000000) >> 24 | (tmp & 0xff000000) << 8 | (tmp & 0xff00000000) >> 8;
    memcpy(*buf, &val, sizeof(int64_t));
    *buf += sizeof(int64_t);
}
...
void write_IEEE_float(SeisOSegy* sgy, char** buf, double val)
{
    ...
    uint32_t fraction = abs_val / pow(2, (int32_t)exp - 127) * pow(2, 23) - 1;
...
void write_IEEE_double(SeisOSegy* sgy, char** buf, double val)
{
    ...
    uint64_t fraction = abs_val / pow(2, (int32_t)exp - 1023) * pow(2, 52 - 1);
