    ...
    if (stanz_num) {
        ...
        if (stanz_num == -1) {
            ...
            if (com->bin_hdr.fixed_tr_length) {
                 ...
                 * headers */
                fseek(com->file,
                    (com->bytes_per_sample * com->samp_per_tr + TRACE_HEADER_SIZE * com->bin_hdr.max_num_add_tr_headers) * com->bin_hdr.num_of_tr_in_file,
                    SEEK_CUR);
                ...
                while (1) {
                    ...
                    char const* last_stanz = seis_common_segy_get_stanza(com, stanz_read - 1);
...
int32_t read_i24_sw(char const** buf)
{
    ...
    memcpy(&res, *buf, sizeof(int16_t) + sizeof(int8_t));
    *buf += sizeof(int16_t) + sizeof(int8_t);
    return (int32_t)((res & 0xff) << 24 | (res & 0xff000000) >> 24 | (res & 0xff00) << 8 | (res & 0xff0000) >> 8);
}
...
uint32_t read_u24_sw(char const** buf)
{
    ...
    memcpy(&res, *buf, sizeof(uint16_t) + sizeof(uint8_t));
    *buf += sizeof(uint16_t) + sizeof(uint8_t);
    return ((res & 0xff) << 24 | (res & 0xff000000) >> 24 | (res & 0xff00) << 8 | (res & 0xff0000) >> 8);
}
...
int64_t read_i64_sw(char const** buf)
{
    ...
    memcpy(&res, *buf, sizeof(int64_t));
    *buf += sizeof(uint64_t);
    return (int64_t)((res & 0xff) << 56 | (res & 0xff00000000000000) >> 56 | (res & 0xff00) << 40 | (res & 0xff000000000000) >> 40 | (res & 0xff0000) << 24 | (res & 0xff0000000000) >> 24 | (res & 0xff000000) << 8 | (res & 0xff00000000) >> 8);
}
...
uint64_t read_u64_sw(char const** buf)
{
    ...
    memcpy(&res, *buf, sizeof(int64_t));
    *buf += sizeof(uint64_t);
    return ((res & 0xff) << 56 | (res & 0xff00000000000000) >> 56 | (res & 0xff00) << 40 | (res & 0xff000000000000) >> 40 | (res & 0xff0000) << 24 | (res & 0xff0000000000) >> 24 | (res & 0xff000000) << 8 | (res & 0xff00000000) >> 8);
}
